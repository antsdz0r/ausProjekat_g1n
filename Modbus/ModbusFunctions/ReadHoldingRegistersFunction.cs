        ...
        public override Dictionary<Tuple<PointType, ushort>, ushort> ParseResponse(byte[] response)
        {
            ...
            if (response[7] != CommandParameters.FunctionCode + 0x80)
            {
                ...
                for (int i = 0; i < response[8]; i += 2)
                {
                    ...
                    PointType responsePointType = ((ModbusReadCommandParameters)CommandParameters).RequestedPointType == PointType.ANALOG_INPUT
                        ? PointType.ANALOG_INPUT
                        : PointType.ANALOG_OUTPUT;

                    r.Add(new Tuple<PointType, ushort>(responsePointType, start_address), value);
